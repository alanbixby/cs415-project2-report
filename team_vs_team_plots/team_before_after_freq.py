...
for game in games:
    ...
    home_tweet_df = home_tweet_df.resample("H").sum().fillna(0)
    ...
    home_reddit_df = home_reddit_df.resample("H").sum().fillna(0)
    ...
    away_tweet_df = away_tweet_df.resample("H").sum().fillna(0)
    ...
    away_reddit_df = away_reddit_df.resample("H").sum().fillna(0)
    ...
    plt.savefig(
        f"frequency/{convert_team_to_index(game['home_team'])}_{convert_team_to_index(game['away_team'])}.png",
        bbox_inches="tight",
    )
    plt.show()
    plt.close()
